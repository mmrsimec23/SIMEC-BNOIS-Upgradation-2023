...
(function () {
    ...
    'use strict';
    var controllerId = 'missionTransfersController';
    angular.module('app').controller(controllerId, missionTransfersController);
    missionTransfersController.$inject = ['$state', '$scope', 'downloadService', 'officerTransferService', 'nominationService','codeValue', 'nominationDetailService', 'employeeService', 'officeAppointmentService', 'notificationService', '$location', 'OidcManager', 'FileUploader'];

    function missionTransfersController($state, $scope, downloadService, officerTransferService, nominationService, codeValue, nominationDetailService, employeeService, officeAppointmentService, notificationService, location,OidcManager,FileUploader) {
        ...
        function init() {
            ...
            officerTransferService.getOfficerTransfer(vm.officerTransferId).then(function (data) {
                nominationService.getNominationByType(2).then(function (data) {
        ...
        function save() {
            vm.officerTransfer.transferFor = 3;
            ...
            if (vm.officerTransferId !== 0 && vm.officerTransferId !== '') {
                updateOfficerTransfer();
            } else {
        ...
        function insertOfficerTransfer() {
            officerTransferService.saveOfficerTransfer(vm.officerTransfer).then(function (data) {
                    ...
                    officerTransferService.getOfficerTransfers(vm.officerTransfer.employeeId, 3).then(function (data) {
...
//                            });
        ...
        function getEmployeeInfo(employeeId) {
            ...
            employeeService.getEmployeeByPno(employeeId).then(function (data) {
                ...
                officerTransferService.getOfficerTransfers(vm.officerTransfer.employee.employeeId, 3).then(
                    function (data) {
                        vm.officerTransfers = data.result.officerTransfers;
                        vm.officerTemporaryTransfers = data.result.officerTemporaryTransfers;
        ...
        function deleteOfficerTransfer(officerTransfer) {
            officerTransferService.deleteOfficerTransfer(officerTransfer.transferId).then(function (data) {
                officerTransferService.getOfficerTransfers(officerTransfer.employeeId, 3).then(function (data) {
                    vm.officerTransfers = data.result.officerTransfers;
                    vm.officerTemporaryTransfers = data.result.officerTemporaryTransfers;
                    if (data.result != null) {
                        $scope.itemValue = false;
        ...
        //----------------------------------Mission Transfer Pdf File Upload----------------------------------
