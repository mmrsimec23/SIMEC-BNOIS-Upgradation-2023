...
(function () {
    ...
    var controllerId = 'toeOfficerListByTransferTypeController'; 

    angular.module('app').controller(controllerId, toeOfficerListByTransferTypeController);
    toeOfficerListByTransferTypeController.$inject = ['$stateParams', '$rootScope', 'dashboardService', 'downloadService','reportService', 'notificationService', '$state'];

    function toeOfficerListByTransferTypeController($stateParams, $rootScope, dashboardService, downloadService, reportService ,notificationService, $state) {
        var vm = this;
        vm.officerDetails = officerDetails;
        vm.toeOfficerListByTransferTypes = [];
        vm.rankId = 0;
        vm.branch = '';
        vm.categoryId = 0;
        vm.subCategoryId = 0;
        ...
        if ($stateParams.transferType !== undefined && $stateParams.transferType !== null) {
            vm.transferType = $stateParams.transferType;
        ...
        Init();
        function Init() {
            dashboardService.getToeOfficerListByTransferType(vm.rankId, vm.branch, vm.categoryId, vm.subCategoryId, vm.commissionTypeId, vm.transferType).then(function (data) {

                vm.toeOfficerListByTransferTypes = data.result;
                vm.totalResult = vm.toeOfficerListByTransferTypes.length;
